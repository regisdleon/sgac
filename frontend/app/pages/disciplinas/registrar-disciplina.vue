...
// Watch para debug
watch(carrerasOrdenadas, (newValue) => {
  console.log('Carreras ordenadas actualizadas:', newValue);
}, { immediate: true });
...
async function onSubmit(event: FormSubmitEvent<SchemaType>) {
  ...
  const result = await disciplineStore.createDiscipline(disciplineData, carreraId);

  if (result) {
    toast.add({
      title: 'Disciplina creada correctamente',
    ...
    >
      <template #header>
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold">Registrar Disciplina</h2>
          <UButton
              variant="ghost"
